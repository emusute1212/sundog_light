...
このリポジトリは公開される前提で扱います。`AGENTS.md`、`README.md`、`docs`配下には、次の情報を記載しません。

- securityに関わる実装方式、通信契約、識別子、状態管理
- 内部インターフェース、接続経路、データ構成の一覧
- メンテナンス制御やアクセス制御の具体的な判定方法
- 本番インフラの識別子、非公開URL、運用runbook
- credential、secret、個人端末の絶対パス

実装確認に必要な情報は、公開文書へ転記せず、対象コードと権限管理された仕様をその都度参照します。
