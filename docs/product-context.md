...
- ホストはイベントの作成、編集、削除、色操作を行う。
- 参加者は共有URLから現在色を取得し、その後の色更新を受け取る。
- フロントエンドの `#RRGGBB` と外部契約の24-bit整数は、既存の境界処理で相互変換する。
- メンテナンス状態と事前告知は、運用側の状態を画面へ反映する。

## 技術選定

- Next.js 16、React 19、TypeScript、Tailwind CSS。
- QRコード生成は `qrcode.react`。
- Google Analytics helper は `src/features/core/components/GoogleAnalytics.tsx` にある。
...
- `currentColor`: 現在色。未選択時は `null`。
- `clientPageUrl`: 任意の参加者URL。フロントエンドでは現在のオリジンを優先して共有URLを組み立てる。

保存方式を変更するときは、サーバー側の公開契約を先に更新し、フロントエンドはその契約に追従します。
...
- ログイン状態が失効した場合は再ログインへ案内し、権限不足やメンテナンス状態はそれぞれ別の画面状態として扱う。
